...
resp = client.create_bucket({
    bucket: bucket_name,                # Name of th bucket
...
number_of_files.times.each do |i|
    ...
    output_path = "/tmp/#{filename}"    # Speicifying the output path for the file

    # Writing a unique UUID for each file
    File.open(output_path, "w") do |f|
        f.write SecureRandom.uuid   # Write a random UUID to the file
    end

    # Open and read the file in binary mode, then upload it to S3
    File.open(output_path, 'rb') do |f|  # 'rb' mode fo reading binary data
